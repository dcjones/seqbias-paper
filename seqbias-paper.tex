...
such as de novo gene annotation and gene or isoform quantification.
...
excluding parameters that are insufficiently informitive in discriminating
...
highly expressed genes, we ignore duplicate reads, which we define as tow reads
mapping to the same location in the genome. The nucleotide sequence is taken
from the genome, rather then the reads themselves, allowing us to include
...
(see Supplementary Section 3), a number which our theoretical results, detailed
in Supplementary Section 10, suggest should lead to accurate estimates. A perhaps
...
the number of parameters that must be estimated (See Supplementary Section 10).
...
few positions adjacent to the read start (see Supplementary Section 8).
